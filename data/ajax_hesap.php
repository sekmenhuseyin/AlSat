...
if($page=="account_stlk"){
		if($p=="stlk_list" or !$p){ //Listelenenler
			...
			<? }
		}else if($p=="stlk_timeout"){ //Zamaný Dolanlar
			$sec="select id,title,KatID,fiyat,hemen,tarih from stok where MusteriID='$_SESSION[UserID]' and durum='1' and trh<='$trh' order by title Asc";$sor=mysql_query($sec);$sat_no=mysql_num_rows($sor);
			if(!$sat_no){echo"Ürün yok !";}else{?>
				<table width="100%" border="0" cellspacing="0" cellpadding="2" align="center">
			...
			<? }
		}else if($p=="stlk_ode"){ //Ücreti Ödenecekler
			...
			<? }
		}else if($p=="stlk_buy"){ //Sattýklarým
		...
		if($p=="tklf_ver" or !$p){ //Verdiðim Teklifler?>
				<table width="100%" border="0" cellspacing="0" cellpadding="2" align="center">
					<tr><th>Resim</th><th>Baþlýk</th><th>Kategori</th><th>Fiyat</th><th>Kalan Süre</th></tr>
		...
		<? }else if($p=="tklf_win"){ //Kazandýðým Teklifler?>
				<table width="100%" border="0" cellspacing="0" cellpadding="2" align="center">
					<tr><th>Resim</th><th>Baþlýk</th><th>Kategori</th><th>Fiyat</th><th>Kalan Süre</th><th>Ýþlem</th></tr>
    				<? $sec="select distinct StokID from stok_teklif where MusteriID='$_SESSION[UserID]'";$sor=mysql_query($sec);while($oku=mysql_fetch_assoc($sor)){
						$en_yuksek="select * from stok_teklif where StokID='$oku[StokID]' order by teklif desc";$en_sor=mysql_query($en_yuksek);$en_oku=mysql_fetch_assoc($en_sor);
		...
		<? }else if($p=="tklf_lose"){ //Kaybettiðim Teklifler?>
				<table width="100%" border="0" cellspacing="0" cellpadding="2" align="center">
					<tr><th>Resim</th><th>Baþlýk</th><th>Kategori</th><th>Fiyat</th><th>Kalan Süre</th></tr>
    				<? $sec="select distinct StokID from stok_teklif where MusteriID='$_SESSION[UserID]'";$sor=mysql_query($sec);while($oku=mysql_fetch_assoc($sor)){
						$en_yuksek="select * from stok_teklif where StokID='$oku[StokID]' order by teklif desc";$en_sor=mysql_query($en_yuksek);$en_oku=mysql_fetch_assoc($en_sor);
				...
				if($istek==dgstr){
					if(($submit) and ($psid==$PHPSESSID)){$guncelle=mysql_query("UPDATE uye_active SET ad='$ad',soyad='$soyad',adres='$adres',tel='$tel',sehir='$sehir',ilce='$ilce' where id='$_SESSION[UserID]'");$bsr="blg";}
						...
						<form name="frmBlg" method="post" action="?page=account_kisi&istek=dgstr"><table width="100%" border="0" cellspacing="0" cellpadding="0" align="center">
							<tr><td width="30%">Kullanýcý Adýnýz</td><td><b><? echo"$oku[username]";?></b></td></tr>
							...
							<tr><td>Bulunduðunuz Þehir</td><td><select size="1" name="sehir"><? $sec_shr=mysql_query("select * from sehir order by plaka asc");
							...
							<tr><td align="center"><input type="button" value="   Ýptal   " name="btn_iptal" onclick="javascript:window.location.href='?page=account_kisi';" /></td><td><input type="submit" name="submit" value="Tamamla" /></td></tr>
						</table></form><br /><br />
				...
				if(!$istek or $bsr){
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center">
							<? if($bsr) {
								...
								<tr><td colspan='2' align='center'><br /></td></tr>
							<? }?>
						<tr><td width="25%">Kullanýcý Adýnýz</td><td><? echo"$oku[username]";?></td></tr>
